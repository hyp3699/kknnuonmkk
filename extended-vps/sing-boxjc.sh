...
install_dependencies() {
    ...
    if ! command -v trans >/dev/null 2>&1; then
        ...
        curl -L --fail --connect-timeout 15 --max-time 60 https://git.io/trans -o /usr/local/bin/trans
        if [ $? -ne 0 ]; then
...
translate_text() {
    ...
    while IFS= read -r line; do
        ...
        result=$(timeout 5 trans -b :zh "$line" 2>/dev/null)
        if [ -n "$result" ]; then
            echo "$result"
        else
            echo "$line"
...
show_latest_start_errors() {
    ...
    TMP_LOG=$(mktemp)
    journalctl -u sing-box --since "$START_TIME" --no-pager > "$TMP_LOG" 2>&1
...
check_configs() {
    ...
    while IFS= read -r file; do
        ...
        find "/etc/sing-box/conf" -maxdepth 1 -type f -name "*.json" -print | sort
    )
    if [ "${#errors[@]}" -eq 0 ]; then
